class Report::UtilisationController < Report::BaseController
  ...
  def not_used_run_eth_query(filter)  
      filter = '1=1' if filter.blank?
      query = Query.find_by_sql([
          "select IsNull(e.order_by_level1,9),e.ethnicity_level1 ethnicity, count(c.id) count\n"+
          ...
          "   and #{filter}\n"+
          "group by IsNull(e.order_by_level1,9),e.ethnicity_level1\n"+
          "order by 1\n",
           @criteria.start_date, @criteria.end_date, @criteria.programme_id])
      query
  end
  ...
  def result
    ...
    for org_id in org_loop do 
      ...
      filter_patients = '1=1'
      filter_patients += "  and p.organisation_id = #{@criteria.organisation_id}\n" unless @criteria.organisation_id.blank?
      ...
      # Utilisation By Ethnicity (Maori/PI-Other)
      ticked=params[:utilisation_by_ethnicity_maori_pi_other]
      session[:report][:utilisation_by_ethnicity_maori_pi_other] = ticked
      if ( ticked )
        query = Query.find_by_sql([
          "select e.order_by_mpi, e.ethnicity_mpi ethnicity, count(c.id) count\n"+
          "from claims c left join fee_schedules s on s.id = c.fee_schedule_id\n"+
          "  left join patients p on p.id = c.patient_id\n"+
          "    left join ethnicities e on e.id = IsNull(p.ethnicity_id,#{Ethnicity::OTHER}) \n"+
          "where c.date_service >= ?  and c.date_service <= ? \n"+
          ...
          "   and s.is_a_entry_service = 1\n"+          
          "   and #{filter}\n"+
          "group by e.order_by_mpi,e.ethnicity_mpi\n"+
          "order by 1\n",
           @criteria.start_date, @criteria.end_date, @criteria.programme_id])
        # Convert Query into array hashes. 
        @results << { :title => 'Utilisation by Ethnicity', :partial_name => 'barchart', 
      ...
      # QUESTION: is this by # services, or by Referral only, or by DISTINCT patients ????
      ...
      # Utilisation By Age Group IMM
      ticked=params[:utilisation_by_age_group_imm]
      session[:report][:utilisation_by_age_group_imm] = ticked
      if ( ticked )
        agerange_field="dbo.agerange_imm(p.date_of_birth,c.date_service)"
        query = Query.find_by_sql([
          "select #{agerange_field} agerange, count(c.id) count\n"+
          "from claims c left join fee_schedules s on s.id = c.fee_schedule_id\n"+
          "  left join patients p on p.id = c.patient_id\n"+
        ...
        @results << { :title => 'Utilisation by Age Group (Imm)', :partial_name => 'barchart', 
                        :options => { },
                        :ss => query.map { |r| [ r.agerange, r.count.to_i ] } }
      end
      ...
      # Utilisation By Age + Month, # MS: 2012-02 Removed as not used ??? (see views/report/utilisation/criteria.html)
     ...
     # { :name => 'Utilisation By Ethnicity/Diabetes', :note => ''  },
      
      # Utilisation By Diabetes
      if ( params[:utilisation_by_ethnicity_and_diabetes])
        session[:report][:utilisation_by_ethnicity_and_diabetes] = true

        if ( @criteria.programme_id != Programme::CVDR  &&  @criteria.programme_id != Programme::DIAB )
          ClaimsData.update_all(
                 # Update Referral data with the type of diabetes from DIAB/CVDR if not recorded. 
                 "diab_type_of_diabetes = \n"+
                 "( select top 1 diab2_data.diab_type_of_diabetes"+
                 "  from claims inv\n"+
                 "   left join claims diab2 on diab2.patient_id = inv.patient_id \n"+
                 "                and ( diab2.programme_id = #{Programme::CVDR} or diab2.programme_id = #{Programme::DIAB})\n"+
                 "    left join claims_data diab2_data on diab2_data.id = diab2.id\n"+
                 "  where inv.id = claims_data.id\n"+
                 "    and diab2_data.diab_type_of_diabetes <> '' \n"+
                 ...
                 "from claims c left join fee_schedules s on s.id = c.fee_schedule_id\n"+
                 "  left join claims_data d on d.id = c.id\n"+
                 "   left join claims diab on diab.patient_id = c.patient_id \n"+
                 "                and ( diab.programme_id = #{Programme::CVDR} or diab.programme_id = #{Programme::DIAB})\n"+
                 "    left join claims_data diab_data on diab_data.id = diab.id\n"+
                 " where c.programme_id = #{@criteria.programme_id} \n"+
                 "   and s.is_a_entry_service = 1\n"+          
                 "   and ( d.diab_type_of_diabetes is null or d.diab_type_of_diabetes = '')\n"+
                 "   and ( diab_data.diab_type_of_diabetes <> '')\n"+
                 ")" )
        end

        query = Query.find_by_sql([
          "select e.ethnicity_level1 ethnicity,e.order_by_level1, d.diab_type_of_diabetes diabetes, count(c.id) count\n"+
          "from claims c left join fee_schedules s on s.id = c.fee_schedule_id\n"+
          ...
          "   and s.is_a_entry_service = 1\n"+          
          "   and #{filter}\n"+
          "group by e.ethnicity_level1,e.order_by_level1,d.diab_type_of_diabetes\n"+
          "order by e.order_by_level1, d.diab_type_of_diabetes \n",
           @criteria.start_date, @criteria.end_date, @criteria.programme_id])

        ss = merge_rcv_ss( query, 'ethnicity', 'diabetes',  'count' )
        @results << { :title => 'Utilisation by ethnicity / diabetes', :partial_name => 'spreadsheet', 
                        :options => { :header => true }, :note => nil,
                        :ss => ss }    
      end

      if ( params[:utilisation_by_gender_and_diabetes])
        session[:report][:utilisation_by_gender_and_diabetes] = true
          
        query = Query.find_by_sql([
          "select g.description gender,d.diab_type_of_diabetes diabetes, count(c.id) count\n"+
          "from claims c left join fee_schedules s on s.id = c.fee_schedule_id\n"+
          "  join patients p on p.id = c.patient_id\n"+
          ...
          "   and s.is_a_entry_service = 1\n"+          
          "   and #{filter}\n"+
          "group by g.description,d.diab_type_of_diabetes\n"+
          "order by g.description, d.diab_type_of_diabetes \n",
           @criteria.start_date, @criteria.end_date, @criteria.programme_id])
          
        ss = merge_rcv_ss( query, 'gender', 'diabetes',  'count' )
        @results << { :title => 'Utilisation by gender / diabetes', :partial_name => 'spreadsheet', 
                        :options => { :header => true }, :note => nil,
                        :ss => ss }    
      ...
      if ( params[:utilisation_by_quintile_and_diabetes])
        session[:report][:utilisation_by_quintile_and_diabetes] = true

        query = Query.find_by_sql([
          "select p.quintile,d.diab_type_of_diabetes diabetes, count(c.id) count\n"+
          "from claims c left join fee_schedules s on s.id = c.fee_schedule_id\n"+
          "  join patients p on p.id = c.patient_id\n"+
          "  left join claims_data d on d.id = c.id\n"+
          "where c.date_service >= ?  and c.date_service <= ? \n"+
          ...
          "   and s.is_a_entry_service = 1\n"+          
          "   and #{filter}\n"+
          "group by p.quintile, d.diab_type_of_diabetes\n"+
          "order by p.quintile, d.diab_type_of_diabetes \n",
           @criteria.start_date, @criteria.end_date, @criteria.programme_id])
          
        ss = merge_rcv_ss( query, 'quintile', 'diabetes',  'count' )
        @results << { :title => 'Utilisation by quintile / diabetes', :partial_name => 'spreadsheet', 
                        :options => { :header => true }, :note => nil,
                        :ss => ss }    
      ...
      if ( ticked )
        query = Query.find_by_sql([
          "select s.description service, e.ethnicity_level1 ethnicity,e.order_by_level1,s.id, count(*) count\n"+
          "from claims c\n"+
          "  join patients p on p.id = c.patient_id\n"+
          "  left join ethnicities e on e.id = IsNull(p.ethnicity_id,#{Ethnicity::OTHER})\n"+
          "  left join fee_schedules s on s.id = c.fee_schedule_id\n"+
          ...
          "   and c.claim_status_id <= 6\n"+
          "   and #{filter}\n"+
          "group by s.description, e.ethnicity_level1,e.order_by_level1,s.id\n"+
          "order by e.order_by_level1, s.id\n",
           @criteria.start_date, @criteria.end_date, @criteria.programme_id])
           
        ss = merge_rcv_ss( query, 'service','ethnicity', 'count' )
      ...
      ticked=params[:services_by_ethnicity_maori_pi_other]
      session[:report][:services_by_ethnicity_maori_pi_other] = ticked
      if ( ticked )
        query = Query.find_by_sql([
          "select s.description service, e.ethnicity_mpi ethnicity,e.order_by_mpi,s.id, count(*) count\n"+
          "from claims c\n"+
          "  join patients p on p.id = c.patient_id\n"+
          "  left join ethnicities e on e.id = IsNull(p.ethnicity_id,#{Ethnicity::OTHER})\n"+
          "  left join fee_schedules s on s.id = c.fee_schedule_id\n"+
          ...
          "   and c.claim_status_id <= 6\n"+
          "   and #{filter}\n"+
          "group by s.description, e.ethnicity_mpi,e.order_by_mpi,s.id\n"+
          "order by e.order_by_mpi, s.id\n",
           @criteria.start_date, @criteria.end_date, @criteria.programme_id])
      ...
      ticked=params[:services_by_age_group]
      session[:report][:services_by_age_group] = ticked
      if ( ticked )
        agerange_field="dbo.agerange_gms(p.date_of_birth,c.date_service)"
        query = Query.find_by_sql([
          "select s.description service, #{agerange_field} agegroup, count(*) count\n"+
          "from claims c\n"+
          "  join patients p on p.id = c.patient_id\n"+
          "  left join ethnicities e on e.id = IsNull(p.ethnicity_id,#{Ethnicity::OTHER})\n"+
          "  left join fee_schedules s on s.id = c.fee_schedule_id\n"+
          ...
          "   and c.claim_status_id <= 6\n"+
          "   and #{filter}\n"+
          "group by s.description, #{agerange_field}\n"+
          "order by #{agerange_field}\n",
           @criteria.start_date, @criteria.end_date, @criteria.programme_id])
           
        ss = merge_rcv_ss( query, 'service','agegroup', 'count' )
          
        @results << { :title => 'Services by Age Group', :partial_name => 'spreadsheet', 
                        :options => { :header => true  }, :note => nil,
                        :ss => ss }
      end

      ticked=params[:services_by_age_group_imm]
      session[:report][:services_by_age_group_imm] = ticked
      if ( ticked )
        agerange_field="dbo.agerange_imm(p.date_of_birth,c.date_service)"
        query = Query.find_by_sql([
          "select s.description service, #{agerange_field} agegroup, count(*) count\n"+
      ...
      #  Services by gender
      ticked=params[:services_by_gender]
      session[:report][:services_by_gender] = ticked
      if ( ticked )
        query = Query.find_by_sql([
          "select s.description service, g.description gender,s.id, count(*) count\n"+
          "from claims c\n"+
          "  join patients p on p.id = c.patient_id\n"+
          "  left join genders g on g.id = p.gender_id\n"+
          "  left join fee_schedules s on s.id = c.fee_schedule_id\n"+
          ...
          "   and c.claim_status_id <= 6\n"+
          "   and #{filter}\n"+
          "group by s.description, g.description,s.id\n"+
          "order by g.description desc, s.id\n",
           @criteria.start_date, @criteria.end_date, @criteria.programme_id])
           
        ss = merge_rcv_ss( query, 'service', 'gender',  'count' )
          
        @results << { :title => 'Services by gender', :partial_name => 'spreadsheet', 
                        :options => { :header => true }, :note => nil,
                        :ss => ss }    
      end         

      #  Services by gender MARK
      ticked=params[:services_by_kilometers]
      session[:report][:services_by_kilometers] = ticked
      if ( ticked )
        query = Query.find_by_sql([
          "select s.description service, sum(d.km_kilometeres) kms\n"+
          "from claims c\n"+
          "  left join claims_data d on d.id = c.id\n" +
          "  left join fee_schedules s on s.id = c.fee_schedule_id\n"+
          "where c.date_service >= ?  and c.date_service <= ? \n"+
        ...
        @results << { :title => 'Services by Kilometeres', :partial_name => 'barchart', 
                        :options => { :header => true }, :note => nil,
                        :ss => query.map { |r| [ r.service, r.kms.to_i ] }  }    
      end         


      ticked=params[:services_by_quintile]
      session[:report][:services_by_quintile] = ticked
      if ( ticked )
        query = Query.find_by_sql([
          "select s.description service, p.quintile,s.id, count(*) count\n"+
          "from claims c\n"+
          "  join patients p on p.id = c.patient_id\n"+
          "  left join fee_schedules s on s.id = c.fee_schedule_id\n"+
          "where c.date_service >= ?  and c.date_service <= ? \n"+
          ...
          "   and c.claim_status_id <= 6\n"+
          "   and #{filter}\n"+
          "group by s.description, p.quintile,s.id\n"+
          "order by p.quintile desc, s.id\n",
           @criteria.start_date, @criteria.end_date, @criteria.programme_id])
           
        ss = merge_rcv_ss( query, 'service', 'quintile', 'count' )
          
        @results << { :title => 'Services by quintile', :partial_name => 'spreadsheet', 
                        :options => { :header => true }, :note => nil,
                        :ss => ss }    
      end
      
      # Programme Specific Fields # MS - Added Feb-2012 - For waitemata and hauraki PHO.
      # ===============
      ticked=params[:programme_specific_fields]
      session[:report][:programme_specific_fields] = ticked
      if ( ticked )
        #MARK
        fields=ProgrammeElement.find(:all, :conditions => [ "programme_id = ? and report_by <> ''",@criteria.programme_id], :order => 'position' )
        logger.info('FIELDS='+fields.length.to_s)
        
        fields.each do |data_element|
          [ data_element.report_by, data_element.report_by_2 ].each do |report_by|
            unless report_by.blank?
              transpose=false
              from_part = "claims c\n"+"  left join claims_data d on d.id = c.id\n";
              report_by_description = ProgrammeElement.report_by_description(report_by)
              column_name_label=DataField.find_by_column_name(data_element.column_name).label;

              if ( report_by.index('claims.')  )
                report_by.gsub!('claims.','c.');
              end
              if ( report_by == 'dbo.agerange_gms'  || report_by == 'dbo.agerange_imm' )  # Special column name
                from_part += "  left join patients p on p.id = c.patient_id\n";
                report_by += '(p.date_of_birth,c.date_service)';
              end
              if ( report_by.index('fee_schedules.')  )
                from_part += "  left join fee_schedules f on f.id = c.fee_schedule_id\n";
                report_by.gsub!('fee_schedules.','f.');
              end
              if ( report_by.index('organisations.')  )
                transpose = !transpose
                from_part += "  left join organisations o on o.id = c.organisation_id\n";
                report_by.gsub!('organisations.','o.');
              end  
              if ( report_by.index('patients.')  )
                from_part += "  left join patients p on p.id = c.patient_id\n";
                report_by.gsub!('patients.','p.');
              end
              if ( report_by.index('genders.')  )
                from_part += "  left join patients p on p.id = c.patient_id\n";
                from_part += "  left join genders g on g.id = p.gender_id\n";
                report_by.gsub!('genders.','g.');
              end
              if ( report_by.index('claims_data.')  )
                report_by.gsub!('claims_data.','d.');
              end
              if ( report_by.index('ethnicities.')  )
                from_part += "  left join patients p on p.id = c.patient_id\n";
                from_part += "  left join ethnicities e on e.id = IsNull(p.ethnicity_id,#{Ethnicity::OTHER})\n";
                report_by.gsub!('ethnicities.','e.');
              end
              
              if ( report_by.index('e.') ) 
                # Is by ethnicity, add in order by order_by into the report-by
                report_by = report_by.gsub('ethnicity_','order_by_') + ', ' + report_by;  
              end
              
              # 1st Assume, simple column + count 
              select_part =  "";
              group_by_part = "" 
              order_by_part = ""
              if ( report_by.index('count(')  )
                # Is a simple count, prepared above
                select_part =  "d.#{data_element.column_name} col1, #{report_by} count";
                group_by_part = "d.#{data_element.column_name}" 
                order_by_part = "2 desc"
              elsif ( report_by.index('sum(')  )
                 # Is a sum - re-work SQL # MS: 2012-02 The sum(*) is not yet used
                select_part =  report_by.sub(/\*/,"d.#{data_element.column_name}") + " sum";
                group_by_part = "" 
                order_by_part = "" 
              else
                # Must be col1, col2 + count(x)
                select_part =  "d.#{data_element.column_name} col1, #{report_by} col2, count(c.id) count";
                group_by_part = "d.#{data_element.column_name}, #{report_by}" 
                order_by_part = "3 desc"
                order_by_part = "2" if ( transpose )
                if ( report_by.index('e.') ||  report_by.index('agerange') ) 
                  # Is by ethnicity, order by column #2
                  order_by_part = "2,1"
                end
            end
            
            # Trap exceptions !!! Since user defined.
            begin

              sql = "select #{select_part}\n" +
                    "from #{from_part}\n" +
                    "where c.date_service >= ?  and c.date_service <= ? \n"+
                    "   and c.programme_id = ?\n"+
                    "   and d.#{data_element.column_name} <> ''\n"+
                    "   and c.claim_status_id <= 6\n"+
                    "   and #{filter}\n";
              sql += "group by #{group_by_part}\n" unless group_by_part.blank?
              sql += "order by #{order_by_part}\n" unless order_by_part.blank?
              logger.info("SQL="+sql)
              query = Query.find_by_sql([sql,
                  @criteria.start_date, @criteria.end_date, @criteria.programme_id]);
               
              if ( report_by.index('count(')  )
                 # Count of something. 
                @results << { :title => "#{report_by_description} of #{column_name_label}", :partial_name => 'barchart', 
                                 :options => {  },
                                 :ss => query.map { |r| [ r.col1, r.count.to_i ] } }
              elsif ( report_by.index('sum(')  )
                # Count of something. Only a sigle row. 
                @results << { :title => "#{report_by_description} of #{column_name_label}", :partial_name => 'table', 
                                 :options => {  },
                                 :ss => query.map { |r| [ column_name_label, r.sum.to_i ] } }
              else
                # Must be 2 dimensional. 
                if ( transpose )
                  ss = merge_rcv_ss( query, 'col2', 'col1',  'count' )
                else
                  ss = merge_rcv_ss( query, 'col1', 'col2',  'count' )
                end
                  
                @results << { :title => "#{column_name_label} by #{report_by_description}", :partial_name => 'spreadsheet', 
                                :options => { :header => true }, :note => nil,
                                :ss => ss }    
              end
            rescue Exception => e
               flash[:alert] = "EXCEPTION with #{column_name_label} - #{report_by_description}"
            end                 
            end
          end
        end
      ...
      if ( ticked )
         ...
         query = Query.find_by_sql([        
          ...
          # query = run_diab_query("#{filter} and d.hba1c_hba1c >= 2.00")
          # ss=merge_rhv_ss(query,'ethnicity','HBA1c Recorded','count',ss);

          # Ethnicity by HBA1c >= 7.0 .. <= 8,  53 .. 64 mmol/mol )
#          query = run_diab_query("#{filter} and d.hba1c_hba1c >= 7.00  and d.hba1c_hba1c <= 8.00")
          query = run_diab_query("#{filter} and (( d.hba1c_hba1c >= 7.00 and d.hba1c_hba1c <= 8.00 ) OR ( d.hba1c_hba1c >= 53.00  and d.hba1c_hba1c <= 64.00 ))")
          ss=merge_rhv_ss(query,'ethnicity','HBA1c >= 7%(53) .. <= 8%(64)','count',ss);
    
          # Ethnicity by HBA1c > 8.0, 64 mmol/mol (Max out at 8..12% because of overlap, and 199 mmol/mol for data entry errors)
#          query = run_diab_query("#{filter} and d.hba1c_hba1c > 8.00")
          query = run_diab_query("#{filter} and (( d.hba1c_hba1c > 8.00 and d.hba1c_hba1c < 13.00 ) OR ( d.hba1c_hba1c > 64.00  and d.hba1c_hba1c < 199.00 ))")
          ss=merge_rhv_ss(query,'ethnicity','HBA1c > 8%(64)','count',ss);
          ...
          # Ethnicity by BP < 130/70  # MS: 2011-11-02 - BP is a new reporting criteria within Waitemata PHO (Ethnici breakdown) also changed. 
          query = run_diab_query("#{filter} and ( d.sbp_systolic_blood_pressure  <= 130 and d.dbp_diastolic_blood_pressure  <= 70 )")
          ss=merge_rhv_ss(query,'ethnicity','BP <= 130/70','count',ss);
      ...
      if ( ticked )
        ...
        query_eligiable = Query.find_by_sql([
          "select e.order_by_cvdr orderby, e.ethnicity_cvdr ethnicity, count(p.id) count,  sum(dbo.cvd_eligible(p.date_of_birth,p.gender_id,p.ethnicity_id, ? )) eligiable\n"+
          "from patients p \n"+
          " left join ethnicities e on e.id = IsNull(p.ethnicity_id,#{Ethnicity::OTHER})\n"+
         ...
         @results << { :title => 'CVDR Assessment Breakdown', :partial_name => 'spreadsheet', 
                          ...
                          :note => 'Eligible population is Maori/PI/Indian (Male 35+,Female 45+), Other (Male 45+,Female 55+), Under 75',
                          :ss =>  ss }
        ...
        ss[-1] << run_cvdr_count("( #{highRISK} and #{onStatin})") # Statin
         ...
         @results << { :title => 'Managment Targets % Level  Breakdown', :partial_name => 'spreadsheet', 
                          :options => { :total => false, :ctotal => false, :rtotal => false },
                          :ss =>  ss }
      ...
      if ( ticked )
        query = Query.find_by_sql([
          ...
          "  count(p.id) popn, \n"+  # Fix-up below. cast() does a truncate NOT round (add 50).!!!
          "  cast(((sum(dbo.cplus_eligible(p.date_of_birth, p.gender_id, p.ethnicity_id, p.quintile, cast('#{@criteria.end_date.to_s(:db)}' as datetime) )) + 50) / 100 ) as integer) elig,\n"+
#          "  ((sum(dbo.cplus_eligible(p.date_of_birth, p.gender_id, p.ethnicity_id, p.quintile, cast('#{@criteria.end_date.to_s(:db)}' as datetime) )) / 100)) elig,\n"+
...
#          "  ((sum(dbo.cplus_eligible(p.date_of_birth, p.gender_id, p.ethnicity_id, p.quintile, cast('#{@criteria.end_date.to_s(:db)}' as datetime) )) / 100)) elig,\n"+
      ...
      if ( ticked )
        ...
        # Get number of brief advice.
        query = Query.find_by_sql([
          "select o.id organisation_id, o.name, o.est_no_patients, count(c.id) count\n"+
          "from claims c\n"+
          "  left join claims_data d on d.id = c.id\n"+
          "  left join organisations o on o.id = c.cost_organisation_id\n"+
          "where c.date_service >= ?  and c.date_service <= ? \n"+
          "   and c.programme_id = ?\n"+
          "   and c.claim_status_id <= 6\n"+
          "   and d.smref_smoking_action_referral not like '%to assess%'\n"+
          "group by o.id, o.name,o.est_no_patients\n"+
          "order by 2\n",
           @criteria.start_date, @criteria.end_date, @criteria.programme_id])
        ss=merge_rhv_ss(query,'name','Advice','count');
        ss=merge_rhv_ss(query,'name','Pts','est_no_patients',ss);

        ss.each_with_index do |row,i|
          if ( i == 0 )
              row[2] ='% popn'
          else
            perage=''
            unless ( row[1].blank? || row[2].blank? )
              count=row[1].to_f
              popn=row[2].to_f
              perage="%.2f" % (count*100.0/popn)
            end
            row[2] = perage
          end
        end
        ...
        query = Query.find_by_sql([
          "select o.id organisation_id, o.name, count(c.id) count\n"+
          "from claims c\n"+
          "  left join claims_data d on d.id = c.id\n"+
          "  left join organisations o on o.id = c.cost_organisation_id\n"+
          "where c.date_service >= ?  and c.date_service <= ? \n"+
          ...
          "   and c.claim_status_id <= 6\n"+
          "   and d.smref_smoking_action_referral not like '%to assess%'\n"+
          "   and d.smref_smoking_action_referral not like '%advice only%'\n"+
          "group by o.id, o.name\n"+
          "order by 2\n",
          @criteria.start_date, @criteria.end_date, @criteria.programme_id])
        ss=merge_rhv_ss(query,'name','Intervention','count', ss );
        ...
        ss.each_with_index do |row,i|
          if ( i == 0 )
            ...
            perage=''
            unless ( row[1].blank? || row[3].blank? )
              popn=row[1].to_f
              count=row[3].to_f
              perage=(count*100.0/popn).to_i
            end
            row << perage
          end
        end
        
        @results << { :title => 'Interventions by Practice', :partial_name => 'spreadsheet', 
                      :note => '',
                        :options => { :ctotal => false, :rtotal => false },
                        :ss => ss }                   
      end
      ...
      if ( ticked )
        ...
        # Poidatary First Assessment's By Risk level By Etnicity
        query = Query.find_by_sql([
        ...
        # Poidatary First Assessment's By Risk level By Gender
        query = Query.find_by_sql([
          "select g.description gender,d.rfiskc_foot_risk_category risk, count(*) count\n"+
          "from claims c\n"+
          "  join patients p on p.id = c.patient_id\n"+
          "  left join genders g on g.id = p.gender_id\n"+
          "  left join claims_data d on d.id = c.id\n"+
          ...
          "   and c.claim_status_id <= 6\n"+
          "   and #{filter}\n"+
          "group by g.description,d.rfiskc_foot_risk_category\n"+
          "order by g.description,d.rfiskc_foot_risk_category\n",
           @criteria.start_date, @criteria.end_date, @criteria.programme_id,
           FeeSchedule::PODA_FIRST_ASSESSMENT, FeeSchedule::PODA_SINGLE_VISIT ])
          
        ss = merge_rcv_ss( query, 'gender', 'risk',  'count' )
          
        @results << { :title => 'Podiatry Risk Levels by gender', :partial_name => 'spreadsheet', 
                        :options => { :header => true }, :note => 'Ist Assessment+Single Visit',
                        :ss => ss }
      ...
      ticked=params[:performance_by_practice5y]
      session[:report][:performance_by_practice5y] = ticked
      if ( ticked )
        # NOTE: Performance excludes from the population, patients, who have declined/dnr's etc
        
        query = Query.find_by_sql([
          "select p.organisation_id, o.name, count(p.id) popn, count(distinct c.patient_id) count,\n"+
          "( CASE count(p.id) WHEN NULL THEN 0 WHEN 0  THEN 0 ELSE ( count(distinct c.patient_id)*100.00) / count(p.id)  END ) perage\n"+
          " \n"+
          "from patients p\n"+
          "  left join organisations o on o.id = p.organisation_id\n"+
          ...
          "  left join fee_schedules f on f.id = c.fee_schedule_id\n"+
          "where p.organisation_id is not null\n"+
          "  and dbo.ageym(p.date_of_birth,?) like ' 5Y%'\n"+
          "  and ( (f.is_a_declined_service = 0 and f.is_a_dnr_service = 0 ) OR\n"+
          "        (c.id is null))\n"+
          "group by p.organisation_id, o.name\n"+
          "order by 5 desc\n",
           @criteria.programme_id,@criteria.end_date ]);

        # Convert Query into array hashes. 
        ss = query.map { |r| [ r.name, r.count.to_s+'/'+r.popn.round.to_s, 
                                   ((r.perage.to_f)*100).to_i/100.00, r.organisation_id.to_i ] }
        @results << { :title => 'Performance by Practice (5y)', :partial_name => 'barchart', 
                      :note => 'NOTE: Declined/DNR patients have been excluded from the population',
                        :options => { :row_count => true },
                        :ss => ss }                   
      end
      ...
      if ( ticked )
        query = Query.find_by_sql([
          ...
          "order by 1 desc\n",
           @criteria.start_date, @criteria.end_date, @criteria.programme_id])
          # Convert Query into array hashes. 
        ss = query.map { |r| [ r.name, r.count.to_i, r.total.to_f.round ] }
        ss.unshift( ['Practice','Count','Amount' ] )
        @results << { :title => 'Total Cost by Practice', :partial_name => 'spreadsheet', 
                      :note => '$\'s',
                        :options => { :rtotal => false },
                        :ss => ss }                   
      end
    end # org_loop 
